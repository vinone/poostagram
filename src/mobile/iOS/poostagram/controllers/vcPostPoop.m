...
{
    NSLog(@"%f", ((totalBytesWritten / totalBytesExpectedToWrite) * 100.0));
}
...
- (void)request:(RKRequest *)request didLoadResponse:(RKResponse *)response
{
    ...
    if ([response isOK]) {
        ...
        NSLog(@"Upload failed with status code: %d", [response statusCode]);
    }
}
...
- (IBAction)postYourPoop:(id)sender {
    ...
    NSURL* gRKCatalogBaseURL = [[NSURL alloc] initWithString:@"http://poostagram.com"];
    RKClient* _client;
    ...
    [_client post:@"/upload" params:params delegate:self];
